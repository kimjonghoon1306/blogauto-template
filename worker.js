...
function getAdminHTML(cfg) {
  ...
  var css = [
    ...
    '.a-sidebar{background:var(--sf);border-right:1px solid var(--br);display:flex;flex-direction:column}',
    ...
    '.sb-btn{background:transparent;border:none;color:var(--t3);font-size:10px;cursor:pointer;padding:3px 5px;font-family:inherit;transition:color .15s}',
    '.sb-btn:hover{color:var(--t1)}',
    '.sb-del:hover{color:var(--rd)}',
  ...
  var js = [
    ...
    '    return "<div class=\\"sb-cat\\"" + (filterCat===c?" class=\\"sb-cat cur\\"":"") + " data-cat=\\""+c+"\\"><span class=\\"sb-cat-name\\">"+c+"</span><div class=\\"sb-cat-acts\\"><button class=\\"sb-btn\\" data-action=\\"edit\\" data-name=\\""+c+"\\">편집</button><button class=\\"sb-btn sb-del\\" data-action=\\"del\\" data-name=\\""+c+"\\">삭제</button></div></div>";',
    '  }).join("");',
    ...
    '  var body={id:document.getElementById("eId").value||undefined,title:title,category:document.getElementById("pCat").value||"일반",tags:document.getElementById("pTags").value.split(",").map(function(t){return t.trim();}).filter(Boolean),thumbnail:document.getElementById("pThumb").value,content:document.getElementById("pContent").value};',
    ...
    '  dl.innerHTML=(d.categories||[]).map(function(c){return "<option value=\\""+c+"\\">";}).join("");}catch(e){}',
    '}',
    'function isLight(hex){var h=hex.replace("#","");var r=parseInt(h.substr(0,2),16)||0,g=parseInt(h.substr(2,2),16)||0,b=parseInt(h.substr(4,2),16)||0;return(0.299*r+0.587*g+0.114*b)/255>0.5;}',
    ...
    '  });',
    '  document.getElementById("sAccent").addEventListener("input",function(){applyAccent(this.value);});',
    '  var cur="' + accent + '";',
    ...
    '  }',
    '  var body={name:document.getElementById("sName").value,desc:document.getElementById("sDesc").value,accent:document.getElementById("sAccent").value,adClient:document.getElementById("sAdClient").value,slotTop:document.getElementById("sSlotTop").value,slotMid:document.getElementById("sSlotMid").value,slotBottom:document.getElementById("sSlotBot").value,webhook:document.getElementById("sWebhook").value};',
    ...
    '  var d=await r.json();',
    '  if(d.ok){',
    '    if(pw1){alert("저장했습니다. 비밀번호가 변경되었습니다. 다시 로그인해주세요.");localStorage.removeItem("adm_tok");location.reload();}',
    ...
    '    if(!d.submissions||!d.submissions.length){el.textContent="신청 내역이 없습니다";return;}',
    '    var rows="";',
    ...
    '      row.className="pi";',
    '      row.innerHTML=',
    '        "<span class=\'pi-cat\' style=\'color:var(--ac)\'>"+s.status+"</span>"+',
    '        "<div class=\'pi-ttl\'>"+s.name+" / "+s.contact+" / "+s.blog_name+"</div>"+',
    '        "<span class=\'pi-dt\'>"+dt+"</span>"+',
    '        "<div class=\'pi-acts\'><button class=\'btn btn-dl btn-sm\' data-id=\'"+s.id+"\'>삭제</button></div>";',
    '      el.appendChild(row);',
    '    });',
    '    el.innerHTML="";',
    '    d.submissions.forEach(function(s){',
    ...
    '      var cat=document.createElement("span");cat.className="pi-cat";cat.style.color="var(--ac)";cat.textContent=s.status;',
    '      var ttl=document.createElement("div");ttl.className="pi-ttl";ttl.textContent=s.name+" / "+s.contact+" / "+s.blog_name;',
    '      var dtEl=document.createElement("span");dtEl.className="pi-dt";dtEl.textContent=dt;',
    '      var btn=document.createElement("button");btn.className="btn btn-dl btn-sm";btn.textContent="삭제";btn.setAttribute("data-id",s.id);',
    '      var acts=document.createElement("div");acts.className="pi-acts";acts.appendChild(btn);',
    '      row.appendChild(cat);row.appendChild(ttl);row.appendChild(dtEl);row.appendChild(acts);',
    '      el.appendChild(row);',
    '    });',
    '    el.onclick=function(e){var b=e.target.closest("[data-id]");if(b)delSub(b.getAttribute("data-id"));};',
    '  }catch(e){}',
    '}',
  ...
  var name = cfg.BLOG_NAME;
  var wh = cfg.WEBHOOK_SECRET;
  var adClient = cfg.ADSENSE_CLIENT;
  var slotTop = cfg.ADSENSE_SLOT_TOP;
  var slotMid = cfg.ADSENSE_SLOT_MID;
  var slotBottom = cfg.ADSENSE_SLOT_BOTTOM;

  var html = '<!DOCTYPE html><html lang="ko" data-theme="dark"><head>' +
    '<meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0">' +
    '<title>관리자 — ' + name + '</title>' +
    '<script>(function(){var t=localStorage.getItem("bt")||"dark";document.documentElement.setAttribute("data-theme",t);})()<' + '/script>' +
    '<link rel="preconnect" href="https://fonts.googleapis.com">' +
    '<link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>' +
    '<link href="https://fonts.googleapis.com/css2?family=Noto+Sans+KR:wght@300;400;500;700;900&display=swap" rel="stylesheet">' +
    '<style>' + commonCSS() + accentCSS(accent) + css + '</style></head><body>' +
    headerHTML(cfg, '<a href="/" target="_blank">블로그</a>') +
    '<div id="loginWrap" class="login-wrap">' +
      '<div class="login-box">' +
        '<div class="login-hd"><div class="login-logo"><em>' + name + '</em></div><div class="login-sub">Admin Access</div></div>' +
        '<div class="login-bd">' +
    ...
    '<div id="adminWrap" class="hidden">' +
      '<div class="a-head"><div class="a-hi">' +
        '<div class="a-logo"><em>' + name + '</em> Admin</div>' +
        '<div class="a-nav">' +
      ...
      '<div class="a-body">' +
        '<div class="a-sidebar">' +
          ...
          '<div class="sb-add"><div class="sb-row">' +
        ...
        '<div class="a-main">' +
          ...
          '</div></div>' +
          '<div id="tabWrite" class="hidden"><div class="w-panel">' +
          ...
          '<div id="tabSettings" class="hidden"><div class="set-panel">' +
            '<div class="set-sec"><div class="set-sec-ttl">기본 설정</div>' +
              '<div class="fg">' +
                '<div><label class="fl">블로그 이름</label><input class="fi" id="sName" value="' + name + '"></div>' +
                '<div><label class="fl">블로그 설명</label><input class="fi" id="sDesc" value="' + cfg.BLOG_DESC + '"></div>' +
              '</div>' +
            '</div>' +
            '<div class="set-sec"><div class="set-sec-ttl">테마 색상</div>' +
              ...
              '</div>' +
            '</div>' +
            '<div class="set-sec"><div class="set-sec-ttl">애드센스 설정</div>' +
              '<div class="fg2"><label class="fl">Publisher ID (ca-pub-...)</label><input class="fi" id="sAdClient" value="' + adClient + '" placeholder="ca-pub-XXXXXXXXXX"></div>' +
              '<div class="fg3">' +
                '<div><label class="fl">상단 슬롯</label><input class="fi" id="sSlotTop" value="' + slotTop + '"></div>' +
                '<div><label class="fl">중간 슬롯</label><input class="fi" id="sSlotMid" value="' + slotMid + '"></div>' +
                '<div><label class="fl">하단 슬롯</label><input class="fi" id="sSlotBot" value="' + slotBottom + '"></div>' +
              '</div>' +
            '</div>' +
            '<div class="set-sec"><div class="set-sec-ttl">웹훅 설정</div>' +
              '<div class="fg2"><label class="fl">Webhook Secret Key</label><input class="fi" id="sWebhook" value="' + wh + '"></div>' +
            '</div>' +
            '<div class="set-sec"><div class="set-sec-ttl">비밀번호 변경</div>' +
